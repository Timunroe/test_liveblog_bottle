...
db = {
    ...
    "pages": [
        {
            "title": "weather nov 9", "page_id": "1", "script_name": "weather-nov-9", "timestamp": "11-09-2018", "DNN_asset": "123456", "posts": [
        ...
        {
            "title": "council nov 4", "page_id": "2", "script_name": "council-nov-4", "timestamp": "11-04-2018", "DNN_asset": "654321", "posts": [
        ...
        {
            "title": "Breaking news nov 6", "page_id": "3", "script_name": "breaking-news-nov-6", "timestamp": "11-06-2018", "DNN_asset": "142536", "posts": [
...
def get_pages():
    pages_list = [{'title': item['title'], 'page_id': item["page_id"], 'Asset#': item['DNN_asset']} for item in db["pages"]]
    return pages_list


def get_posts(page_id, post_id=None):
    if page_id == 'new':
        # dealing with new page, no data to return
        return None
    else:
        # dealing with existing page
        page = next((item for item in db["pages"] if item["page_id"] == page_id))
        if post_id:
            # dealing with new or existing post
            if post_id == 'new':
                # dealing with new post
                return {'page_id': page_id, 'post': None}
            else:
                # dealing with existing post of existing page
                post = next((item for item in page['posts'] if item["post_id"] == post_id))
                return {'page_id': page_id, 'post': post}
        else:
            return page
